class DropBoxController {
    ...
    initEvents() {
        ...
        //abre a janela/modal para selecionar os arquivos
        this.btnSendFileEl.addEventListener('click', event => {
            this.inputFilesEl.click()
        })
        //faz o carregamento dos múltiplos arquivos
        this.inputFilesEl.addEventListener('change', event => {
            //enquanto estiver enviando arquivos desabilita o botão
            this.btnSendFileEl.disabled = true

            // console.log('event.target.files',event.target.files)
            //o 'event' carrega no 'target' as 'files' que por sua vez são enviadas como paramêtro para o método uploadTask
            // ao retornar as promessas, trata essas respostas para serem inseridas no firebase
            this.uploadTask(event.target.files).then(responses => {
                // ao retornar respostas bem-sucedidas deve gravar no firebase
                // o firebase precisa de rotas que funcionam como referências de onde ou o que gravar ou recuperar
                //basicamente o firebase guarda coleções de documentos em formato JSON
                responses.forEach(respost => {
                    // console.log('respost.files[\'input-files\']', respost.files['input-file'])

                    this.getFirebaseRef().push().set(respost.files['input-file'])
                })
                //desabilita barra de progresso, habilita botão enviar, e zera os campos do 'file selector'
    ...
    ajax(url, method = 'GET', formData = new FormData(), onloadstart = function () {}, onprogress = function () {}){
        return new Promise((resolve, reject)=>{
            ...
            //     // console.log(event)
            // }
            //formato compreensível pelo método send do ajax (objeto XMLHttpRequest)
            // let formData = new FormData()
            // //primeiro parametro é o nome, e o segundo é o arquivo
