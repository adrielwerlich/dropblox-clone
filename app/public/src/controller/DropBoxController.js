class DropBoxController {
    ...
    constructor() {
        //evento q monitora a mudança na seleção dos itens
        ...
        this.filesListEl = document.querySelector('#list-of-files-and-directories')
    ...
    connectToFirebase(){
     ...
     var config = {
        apiKey: "AIzaSyACXgNwNiLxMj1g3sgBeaodEmS6VPAhaKA",
        authDomain: "remote-storage-901df.firebaseapp.com",
        databaseURL: "https://remote-storage-901df.firebaseio.com",
        projectId: "remote-storage-901df",
        storageBucket: "remote-storage-901df.appspot.com",
        messagingSenderId: "931051216742"
      };

      firebase.initializeApp(config);

    }
    //verifica quantos elementos estão selecionados!
    getSelectedItems(){
        return this.filesListEl.querySelectorAll('.selected')
    }
    ...
    removeElements(){
        ...
        // this.getSelection().forEach(li => {
            console.log('li.dataset.key', li.dataset.key)

            let file  = JSON.parse(li.dataset.file)

            console.log('data.ref', file.ref)
            console.log('li.dataset', li.dataset)

            let formD = new FormData()
            formD.append('path', file.path)
            formD.append('key', li.dataset.key)
            // promisses.push(this.ajax('/upload','POST', formData)

            promises.push(new Promise((resolve, reject)=>{

                if (file.type === 'folder'){
                    this.removeFolder(file).then(e => {
                    ...
                    this.removeFile(file.ref).then(e => {
        ...
        //return tem q estar fora do laço forEach, servindo ao retorno
        // do método removeElements. Do contrário, estará servindo forEach
        return Promise.all(promises)
    }
    //chamada recursiva
    removeFolder(fileData){
        return new Promisse((resolve,reject)=>{
            ...
            folderRef.on('value', snapshot => {
                ...
                console.log('folderRef.remove();',folderRef.remove())
                folderRef.remove();
                console.log('folderRef.off(\'value\')',folderRef.off('value'))
                folderRef.off('value');
    ...
    removeFile(ref){
        ...
        let storageRef = firebase.storage().ref().child(ref);
    ...
    renderNav(){
        ...
        for(let index = 0 ; index < this.currentFolder.length; index++){
            let folder = this.currentFolder[index]
            ...
            if((index + 1) === this.currentFolder.length){
                ...
                // span.innerHTML = `
                //     <a href="javascript:void(-1)" data-path="${path.join('/')}" class="breadcrumb-segment">${folder}</a>
        ...
        // this.navEl.querySelectorAll('.breadcrumb-segment').forEach(anchor => {
        //     a.addEventListener('click', e => {
        //         this.currentFolder = anchor.dataset.path.split('/')
        //         this.openFolder()
        //     })
        // })
    }

    //configura o botão de enviar arquivos para responder à ação do usuário
    initEvents() {
        ...
        this.btnNewFolderEl.addEventListener('click', e => {
            ...
            if(name){
              this.getFirebaseRef().push().set({
                  name, type:'folder',path:this.currentFolder.join('/')
              })
                this.currentFolder.push(name)
                this.openFolder()
            }
        })
        ...
        this.btnDeleteEl.addEventListener('click', e => {
            ...
            this.removeElements().then(responses => {
        ...
        this.btnRenameEl.addEventListener('click', e => {
            ...
            // os dados vem do dataset como string, são transformados em objetos json
            let file = JSON.parse(li.dataset.file)
        ...
        this.filesListEl.addEventListener('selectionchange', e =>{
            /*
            * Quando clica dentro de um item dispara esse evento 'mudança na seleção/classe' do item
            * repare q a seleção é feita por mudança na classe css da tag, portanto, o evento
            * deve ser disparado após a mudança na classe dos eventos
            */
            // console.log('selection.length0', this.getSelectedItems().length);
        ...
        this.inputFilesEl.addEventListener('change', event => {
            ...
            this.progressBarToggle()

            this.uploadTask(event.target.files).then(responses => {
                responses.forEach(respost => {
                    // console.log('respost.name.typeof',typeof(respost.name), respost.name)
                    // console.log('respost')
                    console.log(respost)
                    // console.log('respost.files')
                    let downRef = respost.ref
                    downRef.getDownloadURL().then(url => {
                        
                        console.log('teste url ...>>> ',url)
                        this.getFirebaseRef().push().set({
                            name: respost.name,
                            ref: this.currentFolder.join('/') + '/' + respost.name,
                            fullPath : url,
                            type: respost.contentType,
                            bucket: respost.bucket,
                            size: respost.size,
                            // files: respost.files['input-file']
                        })
                    
                    })
                    // this.getFirebaseRef().push().set(respost.files['input-file'])
    ...
    ajax(url, method = 'GET', formData = new FormData(), onloadstart = function () {}, onprogress = function () {}){
        ...
        return new Promise((resolve, reject)=>{
            ...
            ajax.onload = event => {
                try {
                    ...
                    console.log(ajax.responseText)
                    resolve(JSON.parse(ajax.responseText), event, ajax)
                } catch (e) {
                    reject(e, ajax)
                }
            }
            //se acontecer algum erro, esse evento é disparado, rejeitando a promessa
            ajax.onerror = event => {
                reject(event)
            }
            ajax.upload.onprogress = onprogress
            ajax.upload.onloadstart = onloadstart
            // onloadstart()
            ajax.send(formData)
        })
    }

    // ao completar o upload, fecha a barra de progresso
    onUploadComplete(){
    ...
    progressBarToggle(show = true) {
        this.snackModalEl.style.display = (show) ? 'block' : 'none'
    }

    //esse método irá receber os arquivos (num vetor) q serão enviados no upload
    uploadTask(files) {
        let promises = [];

        //files não é vetor. É coleção de elementos html, por isso o spread é necessário.
        // O spread permite converter a coleção em array q permite o uso do forEach
        [...files].forEach(file => {


            promises.push(new Promise((resolve, reject)=>{

                let fileRef = firebase.storage().ref(this.currentFolder.join('/'))
                .child(file.name)
                ...
                console.log("this.currentFolder.join('/') ", this.currentFolder.join('/'))
                console.log("this.currentFolder.join('/')+'/' ", this.currentFolder.join('/')+ '/')
                console.log("this.currentFolder.join('/')+'/' + file.name", this.currentFolder.join('/')+ '/'+ file.name)
                let storageRef = firebase.storage().ref().child
                (this.currentFolder.join('/')+ '/' + file.name)
                ...
                uploadTask.on('state_changed',snapshot => {
                    ...
                    storageRef.getMetadata().then((metadata) => {
                        resolve(metadata, storageRef, file)
    ...
    uploadProgress(file , event){
        ...
        let loadedData = event.loaded
        let totalData = event.total

        let porcent = parseInt((loadedData / totalData) * 100)
        let timeleft = ((100 - porcent) * timeSpent) / porcent

        this.nameFileEl.innerHTML = file.name
        this.timeLeftEl.innerHTML = this.formatTimeToHumanReadable(timeleft)
        this.progressBarEl.style.width = `${porcent}%`

        // console.log(file.name, timeSpent, timeleft, porcent)
        console.log(`arquivo ${file.name} já gastou ${timeSpent} milisegundos. Faltam ${timeleft} milisegundos estimados. ${porcent}% concluídos`)
    }

    formatTimeToHumanReadable(duration){
    ...
    //verifica o tipo do arquivo e insere o desenho do ícone de acordo
    verifyIconType(file){
        switch (file.type) {
            case 'folder':
                return `
    ...
    // cria um item na lista na 'view'
    insertListItem(file, key){

        let li = document.createElement('li')
        li.dataset.key = key
        // o file é um objeto, mas dentro do dataset só se aceitam armazenar string, por isso a conversão --stringify--
        li.dataset.file = JSON.stringify(file)

        li.innerHTML = `
            ${this.verifyIconType(file)}
                <div class="name text-center">${file.name}</div>            
        `

        this.configLiEvents(li)

        return li

    }
    // esse método trata do evento de click sobre o list-item
    // para definir precisamente qual dos arquivos deve ser realçado
    configLiEvents(li){
        ...
        let selectionChangeEvent = new Event('selectionchange')
        ...
        li.addEventListener('dblclick', event => {
            ...
            switch (data.type) {
                case 'folder':
                    ...
                    break
                default:
                    console.log('datapath', data.fullPath)
                    window.open(data.fullPath)
            }
        })


        // a arrow fuction define o evento de click
        li.addEventListener('click', e => {
            ...
            for (let x = 0; x < e.path.length; x++) {
                ...
                if (e.path[x].tagName.toLowerCase() === 'li') {
                    _li = e.path[x];
                    break;
                }
            ...
            // seleciona o primeiro elemento/tag
            if (e.shiftKey){
                let firstLi = this.filesListEl.querySelector('.selected')

                //se o primeiro item existir, isso é, se já foi selecionado
                // ALGORITMOS Q SELECIONA A FAIXA DE LIST ITEMS
                if (firstLi) {
                    let indexStart
                    let indexEnd
                    let childNodes = li.parentElement.childNodes
                    // li é o list item atualmente selecionado
                    // busca no elemento pai os nodos. Para cada um faz a verificação
                    childNodes.forEach((el, index) => {
                        // verifica a faixa dos elementos para recuperar o indice do 1º e do último
                        if (firstLi === el) indexStart = index
                        if (li === el) indexEnd = index
                    })

                    let orderedIndex = [indexStart, indexEnd].sort()

                    childNodes.forEach((el, localIndex) => {
                        if (localIndex > orderedIndex[0] && localIndex  <= orderedIndex[1])
                            el.classList.add('selected')
                    })

                    //para mais detalhes verificar definição do onselectionchange no método initEvents()
                    this.filesListEl.dispatchEvent(this.onselectionchange)
                    // retornar true para encerrar a seleção dos elementos
                    //  console.log('selection.length1', this.getSelectedItems().length);
                    return true
                }
            }

            if (!e.ctrlKey){
                this.filesListEl.querySelectorAll('li.selected').forEach(el => {
            ...
            this.filesListEl.dispatchEvent(this.onselectionchange)
            // console.log('selection.length2', this.getSelectedItems().length);
        })
    }
    ...
    getAllFromFirebase(){
        ...
        this.getFirebaseRef().on('value',snapshot => {
            ...
            snapshot.forEach(item => {
                ...
                if (data.type) {
                    this.filesListEl.appendChild(this.insertListItem(data, key))

                }

                //  console.log(`item ${item} || key ${key} || value ${data}`)
                // console.log(item)
                // console.log(data)
            })
        })
